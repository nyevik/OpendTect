...
macro( OD_GET_LINKLIBS )
	...
	set( HDF5_CXX_IMPORTED_LIBRARIES ${HDF5_CXX_LIBRARIES} )
	if ( HDF5_C_LIBRARIES )
	    list( REMOVE_ITEM HDF5_CXX_IMPORTED_LIBRARIES ${HDF5_C_LIBRARIES} )
	endif()
	set( HDF5_CXX_SHARED_LIBRARY ${HDF5_CXX_IMPORTED_LIBRARIES} )
	unset( HDF5_CXX_IMPORTED_LIBRARIES )
    endif()
...
macro( OD_CREATE_HDF5_IMPORTED_TARGET TARGET_NAME LIBS_VAR )
    if ( TARGET ${TARGET_NAME} OR NOT DEFINED ${LIBS_VAR}
	 OR "${${LIBS_VAR}}" STREQUAL "" )
	return()
    endif()

    set( HDF5_IMPORTED_LIBRARIES ${${LIBS_VAR}} )
    list( GET HDF5_IMPORTED_LIBRARIES 0 HDF5_IMPORTED_LOCATION )
    if ( NOT EXISTS "${HDF5_IMPORTED_LOCATION}" )
	unset( HDF5_IMPORTED_LIBRARIES )
	unset( HDF5_IMPORTED_LOCATION )
	return()
    endif()

    # Debian's FindHDF5 fallback returns raw library lists instead of imported
    # targets. Recreate the shared imported targets OpendTect already expects.
    add_library( ${TARGET_NAME} SHARED IMPORTED GLOBAL )
    set_target_properties( ${TARGET_NAME} PROPERTIES
	INTERFACE_INCLUDE_DIRECTORIES "${HDF5_INCLUDE_DIRS}"
	IMPORTED_LOCATION "${HDF5_IMPORTED_LOCATION}" )

    list( REMOVE_AT HDF5_IMPORTED_LIBRARIES 0 )
    if ( HDF5_IMPORTED_LIBRARIES )
	set_target_properties( ${TARGET_NAME} PROPERTIES
	    INTERFACE_LINK_LIBRARIES "${HDF5_IMPORTED_LIBRARIES}" )
    endif()

    unset( HDF5_IMPORTED_LIBRARIES )
    unset( HDF5_IMPORTED_LOCATION )
endmacro(OD_CREATE_HDF5_IMPORTED_TARGET)

macro( OD_FIND_HDF5 )

    if ( ( NOT DEFINED HDF5_ROOT OR "${HDF5_ROOT}" STREQUAL "" ) AND
	 DEFINED ENV{HDF5_ROOT} AND NOT "$ENV{HDF5_ROOT}" STREQUAL "" AND
	 IS_DIRECTORY "$ENV{HDF5_ROOT}" )
	set( HDF5_ROOT "$ENV{HDF5_ROOT}" )
    endif()

    if ( NOT DEFINED HDF5_ROOT OR "${HDF5_ROOT}" STREQUAL "" )
	OD_CLEANUP_HDF5()
	...
	if ( HDF5_FOUND )
		...
		OD_CREATE_HDF5_IMPORTED_TARGET( hdf5::hdf5-shared
					       HDF5_C_SHARED_LIBRARY )
		OD_CREATE_HDF5_IMPORTED_TARGET( hdf5::hdf5_cpp-shared
